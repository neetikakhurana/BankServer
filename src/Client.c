...
int main(int argc, char **argv){
	...
	if(argc < 4)
	{
		fprintf(stderr, "Usage client <hostname> <port number> <timestep> <filename>\n");
		exit(1);
	}
	...
	//open the filename received from teh command line
    ...
    while (fgets(buffer, MAXFILE, myfile) != NULL)
    {
    	...
    	//sending the transaction to server	
    }
    	//time taken by each transaction within a client
		clock_t end = clock();
		time_spent = (double)(end - begin) / CLOCKS_PER_SEC; 
		//printf("Time taken is %f\n",(double)((time_spent/transactions)*1000000)); 
